...
function draftPrescriptionPaymentData(): PrescriptionPaymentData {
  return {
    id: undefined,
...
const PrescriptionPaymentFormInner: PrescriptionPaymentFormPlugin = ({
  ...
  const [draft, setDraft] = useState<PrescriptionPaymentData>(
    draftPrescriptionPaymentData()
  );
  ...
  const onSave = useCallback(async () => {
    ...
    draft.change = changeValue;

    const savePluginData = !!data?.id ? update : insert;
  ...
  }, [draft, data, prescriptionData?.id, amountOutstanding, changeValue]);
        ...
        <InputWithLabelRow
          ...
          Input={
            <CurrencyInput
              ...
              onChangeNumber={e => {
                setDraft({ ...draft, amountPaid: e });
                if (e >= totalToBePaidByPatient)
                  events.setState({ ...events.state, errorMessage: null });
              }}
